...
def sendMail(receivers, title, content, attachments=[]):
    ...
    配置文件：
    该方法使用前需要配置邮件服务器的地址和用户密码，配置文件保存在用户根目录下的.send_mail_profile，具体配置案例如下：
    ... ...
    ...
    try:
        home = expanduser('~')
        profile = path.join(home, '.send_mail_profile')
    ...
    # 创建内容部分的
    # contentPart = MIMEText(content, subtype='html', charset='utf-8')
